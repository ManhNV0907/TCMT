        ...
        if self.task_num ==1:
            ...
            for epoch in range(self.args.epochs_list[self.task_num - 1]):
                ...
                for idx, batch in enumerate(tqdm(cur_loader, desc=f"Training Epoch {epoch}")):
                    ...
                    mtl_output = AUGD(torch.stack([loss_mem_shared_grad, loss_shared_grad, distill_mem_shared_grad]))
                    ...
                    # training_loss = loss + 5*distill_loss + 2*loss_mem + 5*distill_loss_mem
                    # training_loss = 0.7*loss + 0.3*loss_mem 
